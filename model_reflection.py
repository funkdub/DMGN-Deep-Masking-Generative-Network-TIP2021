...
class Model(nn.Module):
    ...
    def forward(self,I):
        ...
        feat_fromR  = self.head_feat(R+I)
        feat_BC     = self.head_feat(B_coarse+I)
        ...
        # mask2
        feat2mask = torch.cat([input,feat_fromR],dim=1)
        mask2refine = self.seperator2(feat2mask)
        mx,mn = torch.max(mask2refine),torch.mean(mask2refine)
        mask2refine = (mask2refine-mn)/(mx-mn)
        # Final B
        B,mask_fine        = self.refine(feat2refine,1-mask2refine) 
        mask = [mask0,mask1,mask2,mask3,mask4,mask5,mask6,mask7]
        mask.extend(mask_fine)
        return R+I,B_coarse+I,B+I,mask_B,mask,1-mask2refine
...
class Refined(nn.Module):
    ...
    def forward(self, x, mask):
        x = self.conv(x)
        a = 0.4
        outB0,mask0 = self.s_m0(x,mask,a*np.power(0.5,0))
        outB1,mask1 = self.s_m1(outB0,mask,a*np.power(0.5,1))
        outB2,mask2 = self.s_m2(outB1,mask,a*np.power(0.5,2))
        outB3,mask3 = self.s_m3(outB2,mask,a*np.power(0.5,3))
        outB4,mask4 = self.s_m4(outB3,mask,a*np.power(0.5,4))
        outB5,mask5 = self.s_m5(outB4,mask,a*np.power(0.5,5))
